...
C
     ...
     *                                      IFLAG,XNORM0,CRIT_CONV)
C
      include 'mpif.h'
      INTEGER N,M,IPRINT(2),IFLAG
      DOUBLE PRECISION X(N),G(N),DIAG(N),W(N*(2*M+1)+2*M)
      DOUBLE PRECISION F,EPS,XTOL, ddX, CRIT_CONV,XNORM0
      LOGICAL DIAGCO
C
...
C
     ...
     .        BOUND,NPT,CP,I,NFEV,INMC,IYCN,ISCN, kk,id,ierr,MPI_COMM_WORLD
      LOGICAL FINISH
C
      SAVE
...
C
  ...
  10  ITER= 0
      ...
      IF((GTOL.LE.1.D-04)) THEN
        IF((LP.GT.0).AND.(id.eq.0)) WRITE(LP,245)
        GTOL=9.D-01
      ENDIF
      NFUN= 1
      POINT= 0
...
C     
      ...
      MAXFEV= 20
C
      IF((IPRINT(1).GE.0).AND.(id.eq.0)) CALL LB1(IPRINT,ITER,NFUN,
...
C     ----------------------------------------------------
 ...
 172  CONTINUE
     ...
     *            XTOL,MAXFEV,INFO,NFEV,DIAG,ddX)
      IF (INFO .EQ. -1) THEN
        IFLAG=1
        RETURN
      ENDIF
...
C      XNORM= 1.0D0+XNORM
      CRIT_CONV=(GNORM/100.d0 + abs(ddX)/XNORM0)
C      IF (GNORM/XNORM .LE. EPS) FINISH=.TRUE.
      IF (CRIT_CONV .LE. EPS) FINISH=.TRUE.
C
      IF(IPRINT(1).GE.0) CALL LB1(IPRINT,ITER,NFUN,
     *               CRIT_CONV,N,M,X,F,G,STP,FINISH)
      IF (FINISH) THEN
         IFLAG=0
         RETURN
      ENDIF
...
C
 190  IFLAG=-1
      IF((LP.GT.0).AND.(id.eq.0)) WRITE(LP,200) INFO
      RETURN
 195  IFLAG=-2
      IF((LP.GT.0).AND.(id.eq.0)) WRITE(LP,235) I
      RETURN
 196  IFLAG= -3
      IF((LP.GT.0).AND.(id.eq.0)) WRITE(LP,240)
C
C     FORMATS
C     -------
C
...
C
 ...
 20   FORMAT('  N=',I5,'   NUMBER OF CORRECTIONS=',I2,
     .       /,  '       INITIAL VALUES')
 30   FORMAT(' F= ',1PD10.3,'   CRITC= ',1PD10.3)
 40   FORMAT(' VECTOR X= ')
...
C     **************************
C
      SUBROUTINE MCSRCH(N,X,F,G,S,STP,FTOL,XTOL,MAXFEV,INFO,NFEV,WA,ddX)
      INTEGER N,MAXFEV,INFO,NFEV
      DOUBLE PRECISION F,STP,FTOL,GTOL,XTOL,STPMIN,STPMAX, ddX, DDOT
